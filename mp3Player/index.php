  ...
  var playerTrack = $("#player-track"),
    ...
    albums = [
      <?php
          ...
          $playlist = simplexml_load_file($base);
          foreach($mp3s as $key=>$mp3){
            $virgula = ($key<($total-1))?',':'';
            $result = $playlist->xpath('/playlist/mp3[url="'.$_SERVER['HTTP_HOST'].'/mp3/'.$mp3.'"]');
            echo "\"".$result[0]->channel."\"".$virgula;
          }
        ?>
    ],
    trackNames = [
    ...
    albumArtworks = [
      <?php
          ...
          $rmv = array_pop($mp3s);
          $total = count($mp3s);
          foreach($mp3s as $key=>$file){
            $virgula = ($key<($total-1))?',':'';
            echo "\"_".($key+1)."\"".$virgula;
          }
        ?>
    ],
    trackUrl = [
